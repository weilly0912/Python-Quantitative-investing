...
price.index.freq is None 
rts1=price.resample('M').mean() #'M'指的是每月最后一天
rts1 
rts2=price.resample('MS').mean() #'MS'指的是每月的第一天
rts2 
...
df1=df.sort_values(by='A',axis=0, ascending=False)
df1.sort_index()
df1
...
df.rank(axis=0,method='max')
df.rank(axis=0,method='min')
df.rank(axis=0,method='dense')
df.rank(axis=1,ascending=False)
...
df3 
df2.append(df3)
pd.concat([df2,df3],join='inner',axis=0)
